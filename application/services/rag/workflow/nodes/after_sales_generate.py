...
def create_after_sales_generate_node(
    ...
    def _build_context(documents: List[Dict[str, Any]]) -> str:
        """构建上下文，每个文档格式化为 'policy_name: content'"""
        if not documents:
            return "无相关文档"
        ...
        for doc in documents:
            ...
            policy_name = doc.get("metadata", {}).get("policy_name", "")
            if policy_name:
                # 按要求格式：policy_name: content
                context_parts.append(f"{policy_name}: {content}")
            else:
    ...
    def generate_answer(state: AgentState) -> Dict[str, Any]:
        """基于售后规则文档生成回答"""
        logger.info(f"[售后规则] 生成回答，相关文档数量: {len(state['relevant_documents'])}")
        ...
        prompt_value: PromptValue = prompt_port.get_prompt(
            "agentic_rag.after_sales_answer_prompt",
            query=state["query"],
            context=context,
        ...
        logger.info(f"[售后规则] 回答生成完成，长度: {len(answer)}")
        return {
            "answer": answer,
            "documents": documents
        }
