"""
文档检索应用服务
负责文档检索流程的协调和编排，不包含纯业务逻辑
"""

from typing import List
from domain.service.document.document_retrieval_service import DocumentRetrievalService
from domain.service.document.document_similarity_service import DocumentSimilarityService, DistanceMetricType
...
class DocumentRetrievalAppService(DocumentRetrievalService):
    """
    文档检索应用服务
    协调文档检索流程，处理基础设施和领域服务之间的交互
    """
    ...
    def __init__(
        ...
        similarity_service: DocumentSimilarityService = None
    ):
        self.vector_store = vector_store
        self.embedding_generator = embedding_generator
        self.logger = logger
        self.default_collection = default_collection
        self.similarity_service = similarity_service or DocumentSimilarityService(metric_type=DistanceMetricType.EUCLIDEAN)
    ...
    def retrieve_similar_documents(self, query: str, limit: int = 5, score_threshold: float = 0.5, collection_name: str = None) -> List[RetrievalResult]:
        ...
        """
        try:
            self.logger.info(f"检索相似文档: query={query[:50]}..., limit={limit}, threshold={score_threshold}")
            ...
            for result in search_results:
                # 使用领域服务计算相似度分数
                similarity_score = self.similarity_service.convert_distance_to_score(result["distance"])

                # 检查是否达到阈值
                if self.similarity_service.is_similar_enough(similarity_score, score_threshold):
    ...
    def retrieve_similar_documents_by_embedding(self, query_embedding: List[float], limit: int = 5, score_threshold: float = 0.5, collection_name: str = None) -> List[RetrievalResult]:
        ...
        """
        try:
            self.logger.info(f"根据向量检索相似文档: embedding_dim={len(query_embedding)}, limit={limit}, threshold={score_threshold}")
            ...
            self.logger.info(f"向量检索完成: 找到 {len(retrieval_results)} 个相关文档")
            return retrieval_results
    ...
    def add_document_to_collection(self, document, collection_name: str = None):
        ...
        """
        try:
            self.logger.info(f"添加文档到检索集合: document_id={getattr(document, 'id', 'unknown')}")
    ...
    def get_recommended_threshold(self, precision_requirement: str = "medium") -> float:
        ...
        Returns:
            推荐的相似度阈值
        """
        return self.similarity_service.get_recommended_threshold(precision_requirement)
    ...
    def get_default_threshold(self) -> float:
        ...
        Returns:
            默认阈值
        """
        return self.similarity_service.get_default_threshold()
